...
export default function Home() {
  ...
  const fetchData = async () => {
    try {
      ...
      setFeatured(mockProjects.filter(p => p.featured))
      setExperiences(mockExperience.map(e => ({
        ...e,
        start_date: e.startDate,
        end_date: e.endDate,
      })))
    ...
    return (
      <PublicLayout>
        <div style={{ backgroundColor: 'var(--color-paper)', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <p className="font-mono text-xs tracking-widest uppercase" style={{ color: 'var(--color-muted)' }}>Memuat...</p>
        </div>
      </PublicLayout>
    )
  }
  ...
  const titleText = settings?.site_tagline || "Crafting Digital Products | with Joy."
  ...
  const isAvailable = profile?.availability === 'open' || profile?.availability === 'freelance'
  ...
  const displayBadges = hardSkills.length > 0 ? hardSkills.slice(0, 3) : defaultBadges
      ...
      <div style={{ backgroundColor: 'var(--color-paper)', minHeight: '100vh' }}>
        ...
        <section className="max-w-[1440px] mx-auto px-6 md:px-16 min-h-[calc(100vh-80px)] flex flex-col justify-center pt-24 pb-12 relative">
          <div className="flex flex-col lg:flex-row items-center justify-between gap-10 lg:gap-12">
            ...
            <div className="flex-1 min-w-0 w-full max-w-2xl">
              ...
              <div className="flex flex-col items-start gap-3.5 mb-6">
                ...
                >
                  ...
                  <span className="font-mono text-xs tracking-widest uppercase font-medium" style={{ color: 'var(--color-ink)', letterSpacing: '0.1em' }}>
                    {availabilityText}
                  </span>
                </div>

                <p className="text-sm md:text-base" style={{ color: 'var(--color-muted)', fontFamily: 'var(--font-sans)' }}>
                  <span className="font-normal">Portfolio by</span> <span className="font-bold" style={{ color: 'var(--color-ink)' }}>Muhammad Azizi Abdillah</span>
                </p>
              </div>
              ...
              {(() => {
                ...
                return lines.map((line: string, i: number) => {
                  ...
                  const shouldItalicizeLast = i === lines.length - 1 && words.length > 1
              ...
              <p
                ...
                style={{ color: 'var(--color-muted)', maxWidth: '520px', fontWeight: 400, whiteSpace: 'pre-line' }}
              >
                {profile?.intro || 'Menikmati setiap proses, tumbuh dari setiap tantangan, dan berkarya dengan penuh antusias.'}
              </p>
              ...
              {/* Stats Bar */}
              <div className="flex gap-8 md:gap-14 pt-6 border-t reveal reveal-delay-3" style={{ borderColor: 'var(--color-border)' }}>
                <div>
                  <p className="font-sans font-bold text-xl md:text-2xl mb-0.5" style={{ color: 'var(--color-ink)' }}>Informatika</p>
                  <p className="font-mono text-xs" style={{ color: 'var(--color-muted)' }}>UNU Yogyakarta</p>
                </div>
                <div>
                  <p className="font-sans font-bold text-xl md:text-2xl mb-0.5" style={{ color: 'var(--color-ink)' }}>4+</p>
                  <p className="font-mono text-xs" style={{ color: 'var(--color-muted)' }}>Proyek Digital &amp; UI/UX</p>
                </div>
                <div>
                  <p className="font-sans font-bold text-xl md:text-2xl mb-0.5" style={{ color: 'var(--color-ink)' }}>3+ Tahun</p>
                  <p className="font-mono text-xs" style={{ color: 'var(--color-muted)' }}>Organisasi &amp; Tim Kreatif</p>
                </div>
              </div>
            </div>
            ...
            <div
              className="w-full lg:w-auto flex justify-center lg:justify-end reveal reveal-delay-2 mt-6 lg:mt-0"
            >
              <div className="relative" style={{ width: '100%', maxWidth: '400px' }}>
                ...
                >
                  ...
                  <div
                    ...
                    style={{ background: 'linear-gradient(to bottom, transparent 65%, rgba(10,10,10,0.12) 100%)' }}
                  />
                </div>
                ...
                {displayBadges.map((skill: any, index: number) => {
                  const configs = [
                    { pos: "top-[10%] -right-3 lg:-right-6", anim: { animationDuration: '4s' }, color: "#10B981" },
                    { pos: "top-[48%] -left-4 lg:-left-8", anim: { animationDuration: '5s', animationDelay: '1s' }, color: "#3B82F6" },
                    { pos: "bottom-[12%] -right-2 lg:-right-4", anim: { animationDuration: '4.5s', animationDelay: '0.5s' }, color: "#F59E0B" }
                  ]
                  const conf = configs[index]
                  return (
                    <div key={skill.id || index} className={`absolute ${conf.pos} z-20 animate-bounce`} style={conf.anim}>
                      <div className="flex items-center gap-2 bg-white px-3.5 py-1.5 shadow-md border" style={{ borderRadius: 'var(--radius-full)', borderColor: 'var(--color-border)' }}>
                        <span className="w-2 h-2 rounded-full" style={{ backgroundColor: conf.color }}></span>
                        <span className="font-sans text-xs font-semibold whitespace-nowrap" style={{ color: 'var(--color-ink)' }}>{skill.name}</span>
                      </div>
                    </div>
                  )
                })}
          ...
          <div className="flex items-center justify-between mb-12">
            <div>
              ...
              <h2
                ...
                style={{ fontSize: 'clamp(1.5rem, 3vw, 2.25rem)', color: 'var(--color-ink)', letterSpacing: '-0.03em' }}
              >
                Karya Pilihan
              </h2>
            </div>
          ...
          <div className="max-w-[1440px] mx-auto px-6 md:px-16 py-20 md:py-28 grid md:grid-cols-3 gap-12 md:gap-16">
            ...
            <div>
              <p
                ...
                style={{ color: 'rgba(247,247,245,0.75)', letterSpacing: '0.14em' }}
              >
                Latar Belakang
              </p>
              ...
              <p className="text-sm leading-relaxed mb-10" style={{ color: 'rgba(247,247,245,0.85)', fontFamily: 'var(--font-sans)', whiteSpace: 'pre-line' }}>
                {profile?.intro || 'Mahasiswa Informatika yang aktif mendalami UI/UX Design dan Front-End Development. Memiliki latar belakang multimedia yang membentuk pemahaman visual yang kuat — dari layout hingga interaksi fungsional.'}
              </p>
              <Link
                to="/about"
            ...
            <div className="flex flex-col">
              ...
              <div className="flex flex-col gap-0">
                {experiences.filter(e => e.type === 'Education').map((step, i, arr) => {
                  const dateText = formatExperienceDate(step.start_date, step.end_date)
                  ...
                  <div key={step.id || i} className="flex gap-4 items-start">
                    <div className="flex flex-col items-center">
                      <div
                        ...
                        style={{ backgroundColor: i === arr.length - 1 ? 'var(--color-paper)' : 'rgba(247,247,245,0.5)' }}
                      />
                      {i < arr.length - 1 && (
                        <div className="w-px flex-1 min-h-12" style={{ backgroundColor: 'rgba(247,247,245,0.2)' }} />
                      )}
                    </div>
                    <div className="pb-8">
                      <div className="flex items-center gap-2 mb-1">
                        <p className="font-sans font-bold text-sm" style={{ color: 'var(--color-paper)' }}>{step.position}</p>
                        <span
                      ...
                      <p className="font-sans text-xs" style={{ color: 'rgba(247,247,245,0.85)' }}>{step.organization}</p>
                      <p className="font-mono text-[10px] mt-1.5 tracking-wider" style={{ color: 'rgba(247,247,245,0.65)' }}>{dateText}</p>
                    </div>
                  </div>
                )})}
              </div>
        ...
        >
          <div className="max-w-[1440px] mx-auto px-6 md:px-16 py-20 md:py-28 text-center">
            ...
            <h2
              ...
              style={{ fontSize: 'clamp(2rem, 6vw, 4.5rem)', letterSpacing: '-0.04em', color: 'var(--color-ink)', lineHeight: 0.95 }}
            >
              SIAP BERKONTRIBUSI<br />
              <span
                style={{
                  fontFamily: 'var(--font-serif)',
                  fontStyle: 'italic',
            ...
            <p className="text-sm md:text-base mb-10 max-w-xl mx-auto" style={{ color: 'var(--color-muted)', fontFamily: 'var(--font-sans)' }}>
              Menikmati setiap proses, tumbuh dari setiap tantangan, dan siap memberikan yang terbaik untuk produk digital Anda.
            </p>
  ...
  return (
    ...
    >
      <span
        ...
        style={{ color: 'var(--color-muted)', letterSpacing: '0.06em', width: '2.5rem' }}
      >
        0{index + 1}
      </span>
      ...
      <div
        ...
        style={{ height: '6.5rem', backgroundColor: 'var(--color-border-light)', borderRadius: 'var(--radius-md)' }}
      >
        {projectCover ? (
          <img
            src={projectCover}
...
function getExpBadge(type: string, position?: string) {
  ...
  if (t === 'education' || t === 'pendidikan') {
    return { label: 'Pendidikan', color: '#60a5fa', bg: 'rgba(59,130,246,0.15)', border: 'rgba(59,130,246,0.35)' }
  }
  if (t === 'project' || t === 'internship' || t === 'magang' || pos.includes('magang') || pos.includes('intern') || pos.includes('produser') || pos.includes('prakerin')) {
    return { label: 'Magang', color: '#4ade80', bg: 'rgba(34,197,94,0.15)', border: 'rgba(34,197,94,0.35)' }
  }
  if (t === 'freelance' || t === 'organization' || t === 'organisasi' || pos.includes('ketua') || pos.includes('koor') || pos.includes('wakil')) {
    return { label: 'Organisasi', color: '#fbbf24', bg: 'rgba(245,158,11,0.15)', border: 'rgba(245,158,11,0.35)' }
  }
  if (t === 'work' || t === 'pekerjaan') {
    return { label: 'Pekerjaan', color: '#c084fc', bg: 'rgba(168,85,247,0.15)', border: 'rgba(168,85,247,0.35)' }
  }
  return { label: type || 'Pengalaman', color: 'rgba(247,247,245,0.7)', bg: 'rgba(247,247,245,0.1)', border: 'rgba(247,247,245,0.2)' }
}
