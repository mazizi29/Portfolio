...
export default function ProjectDetail() {
  ...
  const fetchProject = async () => {
    ...
    try {
      // Fetch all published projects with gallery
      ...
      const projectList = (data && data.length > 0)
        ? data.map((p: any) => ({
            ...p,
        ...
        : mockProjects
      const index = projectList.findIndex((p: any) => p.slug === slug)
      
      if (index === -1) {
        // Also check if matches mockProjects directly
    ...
    return (
      <PublicLayout>
        <div style={{ backgroundColor: 'var(--color-paper)', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <p className="font-mono text-xs tracking-widest uppercase" style={{ color: 'var(--color-muted)' }}>Memuat Proyek...</p>
        </div>
      </PublicLayout>
    )
  }
  ...
  const tools = Array.isArray(project.tools) ? project.tools : 
                (typeof project.tools === 'string' ? project.tools.split(',') : [])
  ...
  return (
    <PublicLayout>
      ...
      <section className="max-w-[1440px] mx-auto px-6 md:px-16 pt-36 md:pt-44 pb-12">
        <Link
          to="/work"
          className="link-underline font-mono text-xs tracking-widest uppercase mb-10 inline-flex items-center gap-1.5"
        ...
        <div className="grid md:grid-cols-3 gap-12 md:gap-16 mb-12">
          ...
          <div className="flex flex-col gap-6 md:border-l md:pl-8" style={{ borderColor: 'var(--color-border)' }}>
            {project.role && (
              <div>
                <p className="font-mono text-xs tracking-widest uppercase mb-2" style={{ color: 'var(--color-muted)', letterSpacing: '0.1em' }}>
                  Peran / Kontribusi
                </p>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {tools.map((t: string) => (
                    <span
                      ...
                      style={{ borderColor: 'var(--color-border)', color: 'var(--color-ink)', borderRadius: 'var(--radius-sm)', backgroundColor: 'var(--color-surface)' }}
                    >
                      {t.trim()}
                    </span>
                  ))}
      ...
      <div
        ...
        style={{ height: 'clamp(300px, 50vh, 620px)', backgroundColor: 'var(--color-border-light)' }}
        onClick={() => project.cover_url && setSelectedImage(project.cover_url)}
      >
        {project.cover_url ? (
          <>
            <img src={project.cover_url} alt={project.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-[1.01]" />
            <div className="absolute inset-0 bg-black/10 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
              <span className="bg-black/75 text-white font-mono text-xs px-4 py-2 rounded-full">
            ...
            {sections.map((sec) => (
              ...
              >
                <div>
                  ...
                  <p className="font-mono text-xs mt-0.5 uppercase tracking-wider" style={{ color: 'var(--color-muted)' }}>
                    {sec.label}
                  </p>
                </div>
      ...
      {gallery.length > 0 && (
        <section className="max-w-[1440px] mx-auto px-6 md:px-16 pb-20">
          <p className="font-mono text-xs tracking-widest uppercase mb-6" style={{ color: 'var(--color-muted)', letterSpacing: '0.14em' }}>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {gallery.map((img: string, i: number) => (
              <div
                ...
                style={{ height: '280px', backgroundColor: 'var(--color-border-light)', borderRadius: 'var(--radius-md)', borderColor: 'var(--color-border)' }}
              ...
              >
                ...
                <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                  <span className="bg-black/75 text-white font-mono text-xs px-3 py-1.5 rounded-full">
                    Perbesar ↗
                  </span>
                </div>
              </div>
          ...
          <div className="w-1/3 flex justify-start">
            {prevProject && (
              <Link to={`/work/${prevProject.slug}`} className="flex flex-col gap-1 items-start group">
                <span className="font-mono text-xs tracking-widest uppercase group-hover:text-ink transition-colors" style={{ color: 'var(--color-muted)', letterSpacing: '0.1em' }}>← Sebelumnya</span>
                <span className="font-sans font-bold text-sm truncate max-w-[200px]" style={{ color: 'var(--color-ink)' }}>{prevProject.title}</span>
              </Link>
            )}
          </div>
          <div className="w-1/3 flex justify-center">
             <Link to="/work" className="font-mono text-xs tracking-widest uppercase link-underline text-center" style={{ color: 'var(--color-muted)', letterSpacing: '0.1em' }}>
               Semua Karya
             </Link>
          </div>
          <div className="w-1/3 flex justify-end text-right">
            {nextProject && (
              <Link to={`/work/${nextProject.slug}`} className="flex flex-col gap-1 items-end group">
                <span className="font-mono text-xs tracking-widest uppercase group-hover:text-ink transition-colors" style={{ color: 'var(--color-muted)', letterSpacing: '0.1em' }}>Berikutnya →</span>
                <span className="font-sans font-bold text-sm truncate max-w-[200px]" style={{ color: 'var(--color-ink)' }}>{nextProject.title}</span>
              </Link>
            )}
          </div>
        </div>
      ...
      {selectedImage && (
        ...
        >
          <div className="relative max-w-5xl max-h-[90vh] flex flex-col items-center">
            <button
              onClick={() => setSelectedImage(null)}
              className="absolute -top-10 right-0 text-white font-mono text-sm px-3 py-1 rounded bg-white/20 hover:bg-white/40"
            >
              Tutup ✕
            </button>
            <img
