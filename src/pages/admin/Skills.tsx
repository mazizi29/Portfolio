...
export default function AdminSkills() {
  ...
  const handleDragEnter = (e: React.DragEvent, targetId: string) => {
    ...
    setSkills(prev => {
      ...
      if (copy[draggedIndex].category !== copy[targetIndex].category) return prev
  ...
  const handleDragEnd = async (e: React.DragEvent, id: string) => {
    ...
    try {
      const updates = catSkills.map((s, index) => {
        ...
        s.order_index = index;
        return supabase.from('skills').update({ order_index: index }).eq('id', s.id)
      })
        ...
        {loading ? (
          ...
          return (
            <div key={cat} className="mb-8">
              ...
              >
                {catSkills.length === 0 ? (
                  <p className="text-xs px-6 py-5" style={{ color: 'var(--color-muted)' }}>Belum ada data.</p>
                ) : catSkills.map((s) => (
                  <div
                    key={s.id}
                    id={`skill-${s.id}`}
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <span className="text-xl" style={{ color: 'var(--color-muted)', cursor: 'grab' }}>≡</span>
                      <p className="text-sm" style={{ color: 'var(--color-ink)' }}>{s.name}</p>
                    </div>
                    <button
