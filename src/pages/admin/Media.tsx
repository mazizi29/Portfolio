    ...
    <AdminLayout title="Media Library">
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <h2 className="font-sans font-semibold text-xl" style={{ color: 'var(--color-ink)', letterSpacing: '-0.02em' }}>
            Media Library
          </h2>
          <label
            className={`px-5 py-2.5 text-xs font-semibold tracking-widest uppercase cursor-pointer transition-opacity ${uploading ? 'opacity-50 pointer-events-none' : ''}`}
        ...
        <div className="mb-6">
          <input
            ...
            style={{ borderColor: 'var(--color-border)', color: 'var(--color-ink)', borderRadius: 'var(--radius-md)', fontFamily: 'var(--font-sans)' }}
          />
        </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            {filtered.map((item) => (
              <div
                ...
                style={{ borderColor: selected?.id === item.id ? 'var(--color-ink)' : 'var(--color-border)', borderRadius: 'var(--radius-md)' }}
                onClick={() => setSelected(item.id === selected?.id ? null : item)}
              >
                <div className="aspect-video overflow-hidden" style={{ backgroundColor: 'var(--color-border-light)' }}>
                  <img src={item.url} alt={item.name} className="w-full h-full object-cover" loading="lazy" />
                </div>
                <div className="px-3 py-2.5" style={{ backgroundColor: 'var(--color-surface)' }}>
                  <p className="text-xs font-medium truncate" style={{ color: 'var(--color-ink)' }}>{item.name}</p>
                  <p className="text-xs" style={{ color: 'var(--color-muted)' }}>{item.size}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {selected && (
          <div
            ...
            style={{ borderColor: 'var(--color-border)', backgroundColor: 'var(--color-surface)', borderRadius: '0', ...(window.innerWidth >= 768 ? { borderRadius: 'var(--radius-lg)' } : {}) }}
          >
            <div className="flex justify-between items-start mb-2">
               <p className="font-sans font-medium text-sm truncate pr-2" style={{ color: 'var(--color-ink)' }}>{selected.name}</p>
               <button onClick={() => setSelected(null)} className="text-xs shrink-0 px-1 py-1" style={{ color: 'var(--color-muted)' }}>✕</button>
            </div>
            <p className="text-xs mb-4" style={{ color: 'var(--color-muted)' }}>{selected.size}</p>
