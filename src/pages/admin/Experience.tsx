...
export default function AdminExperience() {
  ...
  const handleSave = async (data: Partial<Exp>) => {
    ...
    try {
      ...
      // Work -> Work
      const safeType = 
        data.type === 'Internship' ? 'Project' :
        data.type === 'Organization' ? 'Freelance' :
        (data.type || 'Work')
      ...
      if (editing) {
        ...
        setItems((prev) => sortExperiencesChronological(prev.map((e) => e.id === editing.id ? updatedData : e)))
      } else {
        ...
        if (error) throw error
        if (insertedData) {
          setItems((prev) => sortExperiencesChronological([insertedData, ...prev]))
        }
      }
      setShowForm(false)
      setEditing(null)
              ...
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <p className="text-sm font-medium" style={{ color: 'var(--color-ink)' }}>{exp.position}</p>
                    <span
                  ...
                  <p className="text-xs leading-relaxed" style={{ color: 'var(--color-muted)' }}>{exp.description}</p>
                </div>
...
export function sortExperiencesChronological(items: any[]): any[] {
  const isOngoing = (item: any) => {
    ...
    return !end || end === 'present' || end === 'sekarang' || end === 'Sekarang' || end === 'SEKARANG'
  }
...
function ExpForm({
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      <div className="flex flex-col gap-1.5">
        <label className="font-mono text-xs tracking-widest uppercase" style={{ color: 'var(--color-muted)', letterSpacing: '0.1em' }}>Type</label>
        <select
          value={form.type}
      ...
      <div className="col-span-2 flex flex-col gap-1.5">
        <label className="font-mono text-xs tracking-widest uppercase" style={{ color: 'var(--color-muted)', letterSpacing: '0.1em' }}>Description</label>
        <textarea
          rows={3}
